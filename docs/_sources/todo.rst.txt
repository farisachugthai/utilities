...
syntax highlighting
^^^^^^^^^^^^^^^^^^^^^^
Give the snippet below a shot and see if you can't instantiate a :mod:`Sphinx`
instance and build the docs that way.
...
Will print off all the :class:`traitlets.Token()` configs. Figure out how to
return the global instance and then we have a gruvbox :mod:`pygments` file on Termux. Override to
your hearts content.
