#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Would it shock you if I said this was taken right from the IPython repo?

Sorry but I'm just actually excited to run this setup function and get ``%``
roles! Defintely going to be convenient.
...
def setup(app):
    app.add_object_type('magic', 'magic', 'pair: %s; magic command',
                        parse_magic)
    app.add_role_to_domain('std', 'magic', LineMagicRole(), override=True)

    app.add_object_type('cellmagic', 'cellmagic', 'pair: %s; cell magic',
                        parse_cell_magic)
